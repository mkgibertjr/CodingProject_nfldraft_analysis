...
pickEX <- mastertable %>% group_by(Rnd,Pick) %>%
  summarize(ExDrAv = median(DrAV,na.rm=TRUE), npicks = length(DrAV))

```

```{r}

bestpositions <- mastertable %>% group_by(Pos) %>%
