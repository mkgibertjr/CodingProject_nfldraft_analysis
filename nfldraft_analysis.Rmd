...
summarytable <- mastertable %>%
  group_by(Tm) %>%
  summarise(hits = sum(hit,na.rm=TRUE),picks = length(hit),hit.percentage = hits/picks,totalvalue = sum(dif,na.rm=TRUE),best.pick=Player[[max(dif)]])
